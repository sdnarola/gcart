...
?>
	...
	?>
											<div class="col-sm-6">
												<div class="reviews">
													<a  class="lnk"><?= "(".$comments ?> <?php _el('comments') ?><?= ")" ?></a>
												</div>
											</div>
	<?php
										}
...
<?php
										...
										if(!empty($comments_data))
										{
											...
											foreach ($comments_data as $key => $comments_msg) 
											{

												$review_product_id=$comments_msg['product_id'];
												$to_date     = strtotime(date("Y-m-d h:i:sa"));
												$date        = $comments_msg['add_date'];
...
?>
											<div class="review">
												<div class="review-title"><span class="summary">"<?= $comments_msg['comment'];?>"</span>
<?php
												if($days >0)
												{
?>
...
<?php 
                  ...
                  $totalpages=floor($comments/$limit);
                   if($comments%$limit!=0) 
                    { $totalpages++; }
                     // 
