...
class Categories extends Frontend_Controller
{
	...
	/**
	 * [index description]
	 * @param  string $categoryslug      [Categories slug]
	 * @param  string $sub_category_slug [sub category slug]
	 * @return [All category page data]  
	 */
	public function index($categoryslug = '', $sub_category_slug = '')
	{
		...
		if (!empty($categoryslug) || !empty($sub_category_slug))
		{
			//get the category id from it
			$category     = $this->category->get_category_by_slug($categoryslug);
			...
			if (!empty($pricerange))
			{
				...
				$where['new_price >='] = $prices[0];
				$where['new_price <='] = $prices[1];
			}
